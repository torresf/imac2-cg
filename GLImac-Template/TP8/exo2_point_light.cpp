...
int main(int argc, char** argv) {
	...
	while(!done) {
		...
		glm::vec3 lightPos(2, 4*(glm::cos(windowManager.getTime()) * glm::sin(windowManager.getTime())), 0);
		...
		glUniformMatrix4fv(earthProgram.uMVMatrix, 1, GL_FALSE, 
			glm::value_ptr(lightMVMatrix));
		glUniformMatrix4fv(earthProgram.uNormalMatrix, 1, GL_FALSE, 
			glm::value_ptr(glm::transpose(glm::inverse(lightMVMatrix))));
		glUniformMatrix4fv(earthProgram.uMVPMatrix, 1, GL_FALSE, 
			glm::value_ptr(projMatrix * lightMVMatrix));
		...
		for (int i = 0; i < nbLunes; ++i)
		{
			...
			glUniform3f(uKs, randomColor[i] * .8, .05, randomColor[i]*.2);
